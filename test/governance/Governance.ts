...
describe("Governance - createProposal()", function () {
    beforeEach("Deploy governance", async function (){
        this.governance = await ethers.deployContract("Governance");
    })
    it("Should revert when value is lower than proposalFee", async function () {
        const fee = await this.governance.proposalFee();
        await expect(this.governance.createProposal(randomAddressString(), { value: fee-1n }))
            .to.be.revertedWith("paid proposal fee is wrong");
    });
    it("Should revert when proposed address is zero", async function () {
        await expect(this.governance.createProposal(ethers.ZeroAddress, { value: await this.governance.proposalFee() }))
            .to.be.revertedWith("empty proposal address");
    });
    it("Should revert when no options are set", async function () {
        const explicitProposal = await ethers.deployContract("CallExecutableProposal");
        await expect(this.governance.createProposal(await explicitProposal.getAddress(), { value: await this.governance.proposalFee() }))
            .to.be.revertedWith("proposal options are empty - nothing to vote for");
    });
    it("Should revert when no options are set", async function () {
        const explicitProposal = await ethers.deployContract("CallExecutableProposal");
        await expect(this.governance.createProposal(await explicitProposal.getAddress(), { value: await this.governance.proposalFee() }))
...
describe("VoteBookKeeper test", function () {
    ...
    it("checking votekeeper proposals cap", async function () {
        await this.votebook.initialize(this.defaultAcc.getAddress(), this.fakeGov.getAddress(), 1);
        expect(await this.votebook.owner()).to.equal(this.defaultAcc);
        await this.fakeGov.reset(this.defaultAcc, this.defaultAcc);
        await this.votebook.onVoted(this.defaultAcc, this.defaultAcc, 1);
        await expect(this.votebook.onVoted(this.defaultAcc, this.defaultAcc, 2)).to.be.revertedWith("too many votes");
        await this.votebook.onVoted(this.defaultAcc, this.defaultAcc, 1);
        await this.votebook.setMaxProposalsPerVoter(2);
        await this.votebook.onVoted(this.defaultAcc, this.defaultAcc, 2);
        await expect(this.votebook.onVoted(this.defaultAcc, this.defaultAcc, 3)).to.be.revertedWith("too many votes");

        await this.fakeGov.reset(this.defaultAcc, this.defaultAcc);
        await this.fakeGov.setOutdatedProposals([2]);
        await this.votebook.onVoted(this.defaultAcc, this.defaultAcc, 3);

        expect(await this.fakeGov.failed()).to.equal(false);
        expect(await this.votebook.getProposalIDs(this.defaultAcc, this.defaultAcc)).to.be.deep.equal([1n, 3n]);
        expect(await this.fakeGov.getRecounted()).to.be.deep.equal([1n]);
        expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 1)).to.be.equal(1n);
        expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 2)).to.be.equal(0n);
        expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 3)).to.be.equal(2n);
        ...
        expect(await this.fakeGov.failed()).to.equal(false);
        expect(await this.votebook.getProposalIDs(this.defaultAcc, this.defaultAcc)).to.be.deep.equal([3n, 2n]);
        expect(await this.fakeGov.getRecounted()).to.be.deep.equal([3n]);
        expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 1)).to.be.equal(0n);
        expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 2)).to.be.equal(2n);
        expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 3)).to.be.equal(1n);
    });

    it("checking votekeeper pruning outdated votes", async function () {
        await this.votebook.initialize(this.defaultAcc.getAddress(), this.fakeGov.getAddress(), 1000);
        expect(await this.votebook.owner()).to.equal(this.defaultAcc);
        {
            await this.votebook.onVoted(this.defaultAcc, this.defaultAcc, 1);
            await this.votebook.onVoted(this.defaultAcc, this.defaultAcc, 2);
            ...
            await this.fakeGov.reset(this.defaultAcc, this.defaultAcc);
            await this.votebook.recountVotes(this.defaultAcc, this.defaultAcc);
            expect(await this.fakeGov.failed()).to.equal(false);
            expect(await this.votebook.getProposalIDs(this.defaultAcc, this.defaultAcc)).to.be.deep.equal([1n, 2n, 3n]);
            ...
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 1)).to.be.equal(0n);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 2)).to.be.equal(0n);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 3)).to.be.equal(0n);
        }
    });

    it("checking votes bookkeeper indexes", async function () {
        await this.votebook.initialize(this.defaultAcc.getAddress(), this.fakeGov.getAddress(), 1000);
        expect(await this.votebook.owner()).to.equal(this.defaultAcc);
        {
            await this.votebook.onVoted(this.defaultAcc, this.defaultAcc, 1);
            expect(await this.votebook.getProposalIDs(this.defaultAcc, this.defaultAcc)).to.be.deep.equal([ 1n ]);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 1)).to.be.equal(1n);
            // duplicate vote
            await this.votebook.onVoted(this.defaultAcc, this.defaultAcc, 1);
            expect(await this.votebook.getProposalIDs(this.defaultAcc, this.defaultAcc)).to.be.deep.equal([ 1n ]);
            ...
            await this.votebook.onVoteCanceled(this.defaultAcc, this.defaultAcc, 1);
            expect(await this.votebook.getProposalIDs(this.defaultAcc, this.defaultAcc)).to.be.empty;
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 1)).to.be.equal(0n);
...

        {
            ...
            await this.votebook.onVoted(this.defaultAcc, this.defaultAcc, 2);
            await this.votebook.onVoted(this.defaultAcc, this.defaultAcc, 3);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 1)).to.be.equal(1n, 2n, 3n);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 1)).to.be.equal(1n);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 2)).to.be.equal(2n);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 3)).to.be.equal(3n);
            ...
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 2)).to.be.equal(2n);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 3)).to.be.equal(1n);
            await this.votebook.onVoteCanceled(this.defaultAcc, this.defaultAcc, 2);
            await this.votebook.onVoteCanceled(this.defaultAcc, this.defaultAcc, 3);
            expect(await this.votebook.getProposalIDs(this.defaultAcc, this.defaultAcc)).to.be.empty;
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 1)).to.be.equal(0n);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 2)).to.be.equal(0n);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 3)).to.be.equal(0n);
        }
        {
            ...
            // in reverse order
            await this.votebook.onVoteCanceled(this.defaultAcc, this.defaultAcc, 3);
            expect(await this.votebook.getProposalIDs(this.defaultAcc, this.defaultAcc)).to.be.deep.equal([1n, 2n]);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 1)).to.be.equal(1n);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 2)).to.be.equal(2n);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 3)).to.be.equal(0n);
            await this.votebook.onVoteCanceled(this.defaultAcc, this.defaultAcc, 2);
        ...
        }
        {
            // different accounts
            await this.votebook.onVoted(this.defaultAcc, this.defaultAcc, 1);
            await this.votebook.onVoted(this.defaultAcc, this.otherAcc, 2);
            await this.votebook.onVoted(this.otherAcc, this.defaultAcc, 3);
            expect(await this.votebook.getProposalIDs(this.defaultAcc, this.defaultAcc)).to.be.deep.equal([ 1n ]);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 1)).to.be.equal(1n);
            expect(await this.votebook.getProposalIDs(this.defaultAcc, this.otherAcc)).to.be.deep.equal([ 2n ]);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.otherAcc, 1)).to.be.equal(0n);
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.otherAcc, 2)).to.be.equal(1n);
            expect(await this.votebook.getProposalIDs(this.otherAcc, this.defaultAcc)).to.be.deep.equal([ 3n ]);
            expect(await this.votebook.getVoteIndex(this.otherAcc, this.defaultAcc, 3)).to.be.equal(1n);

            await this.votebook.onVoteCanceled(this.defaultAcc, this.defaultAcc, 1);
            await this.votebook.onVoteCanceled(this.defaultAcc, this.otherAcc, 2);
            await this.votebook.onVoteCanceled(this.otherAcc, this.defaultAcc, 3);
            expect(await this.votebook.getProposalIDs(this.defaultAcc, this.defaultAcc)).to.be.empty;
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.defaultAcc, 1)).to.be.equal(0n);
            expect(await this.votebook.getProposalIDs(this.defaultAcc, this.otherAcc)).to.be.empty;
            expect(await this.votebook.getVoteIndex(this.defaultAcc, this.otherAcc, 1)).to.be.equal(0n);
            expect(await this.votebook.getProposalIDs(this.defaultAcc, this.otherAcc)).to.be.empty;
            expect(await this.votebook.getVoteIndex(this.otherAcc, this.defaultAcc, 1)).to.be.equal(0n);
        }
    });
})
...
describe("Governance test", function () {
    ...
    // todo does not work
    // it("checking creation with a factory", async function () {
    //     const pType = 1n;
    //     const plaintextFactory = await ethers.getContractFactory("PlainTextProposal");
    //     await this.verifier.addTemplate(pType, "plaintext", this.verifierAddress, NonExecutableType, ethers.parseEther("0.4"), ethers.parseEther("0.6"), [0, 1, 2, 3, 4], 120, 1200, 30, 30);
    //     const option = ethers.encodeBytes32String("opt");
    //     const proposal = await plaintextFactory
    //         .connect(this.otherAcc)
    //         .deploy(
    //             "plaintext",
    //             "plaintext-descr",
    //             [option],
    ...
    //     expect(await proposal.owner()).to.equal(await this.otherAcc.getAddress());
    //     expect(await proposal.name()).to.equal("plaintext");
    //     expect(await proposal.description()).to.equal("plaintext-descr");
    //
    //     const externalProposal = await ethers.deployContract("PlainTextProposal", ["plaintext-descr", [option], ethers.parseEther("0.5"), ethers.parseEther("0.6"), 30, 121, 1199, this.verifierAddress]);
    //     await expect(this.gov.createProposal(externalProposal.getAddress(), {value: this.proposalFee})).to.be.revertedWith("proposal contract failed verification");
    // })
    ...
    describe("checking votes for a self-voter", async function () {
        ...
        it("cancel vote via recounting from votebook", async function () {
            this.govable.unstake(this.defaultAcc, ethers.parseEther("10.0"));
    ...
    it("checking execution expiration", async function () {
        ...
        expect(proposalStateInfo.winnerOptionID).to.be.equal(0n);
        expect(proposalStateInfo.votes).to.be.equal(ethers.parseEther("10.0"));
        expect(proposalStateInfo.status).to.be.equal(8n);
    ...
    it("checking proposal cancellation", async function () {
        ...
        expect(proposalStateInfoAfterTask.status).to.be.equal(4n);
