...
contract(
  ...
  async ([
    ...
    beforeEach(async () => {
      ...
      await this.gov.initialize(this.govable.address, this.verifier.address);
    ...
    it('checking creation of a plaintext proposal', async () => {
      ...
      const wrongCode = await AlteredPlainTextProposal.new(
        'plaintext',
        'plaintext-descr',
        [option],
        ratio('0.5'),
    ...
    const createProposal = async (
      ...
      const contract = await ExecLoggingProposal.new(
        'logger',
        'logger-descr',
        options,
        minVotes,
      ...
      await this.gov.createProposal(contract.address, {
        value: this.proposalFee
      });

      return {
        proposalID: await this.gov.lastProposalID(),
        proposal: contract
      };
    };
    ...
    it('checking self-vote creation', async () => {
      ...
      // make new vote
      await expectRevert(
        this.gov.vote(defaultAcc, proposalID, choices),
      ...
      await expectRevert(
        this.gov.vote(defaultAcc, proposalID.add(new BN(1)), choices),
        'proposal with a given ID doesnt exist'
      );
      await expectRevert(
        this.gov.vote(defaultAcc, proposalID, [new BN(3), new BN(4)]),
        'wrong number of choices'
      );
      await expectRevert(
        this.gov.vote(defaultAcc, proposalID, [
          new BN(5),
          new BN(3),
          new BN(4)
        ]),
    ...
    describe('checking votes for a self-voter', async () => {
      ...
      it('checking voting state', async () => {
        ...
        expect(proposalStateInfo.winnerOptionID).to.be.bignumber.equal(
          new BN(0)
        );
      ...
      it('cancel vote', async () => {
        await expectRevert(
          this.gov.cancelVote(defaultAcc, proposalID.add(new BN(1))),
          "doesn't exist"
        );
        await expectRevert(
          this.gov.cancelVote(otherAcc, proposalID),
          "doesn't exist"
        );
        await this.gov.cancelVote(defaultAcc, proposalID);
      ...
      it('recount vote', async () => {
        await this.govable.stake(defaultAcc, ether('5.0'));
        await expectRevert(
          this.gov.recountVote(otherAcc, defaultAcc, proposalID, {
            from: otherAcc
          }),
          "doesn't exist"
        );
        await expectRevert(
          this.gov.recountVote(defaultAcc, otherAcc, proposalID, {
            from: otherAcc
          }),
          "doesn't exist"
        );
        await this.gov.recountVote(defaultAcc, defaultAcc, proposalID, {
          from: otherAcc
        }); // anyone can send
        await expectRevert(
          this.gov.recountVote(defaultAcc, defaultAcc, proposalID, {
            from: otherAcc
          }),
          'nothing changed'
        );
      ...
      afterEach('checking state is empty', async () => {
        ...
        expect(proposalStateInfo.status).to.be.bignumber.equal(new BN(0));
        const voteInfo = await this.gov.getVote(
          defaultAcc,
          defaultAcc,
          proposalID
        );
    ...
    it('checking voting tally for a self-voter', async () => {
      ...
      expect(await proposalContract.executedCounter()).to.be.bignumber.equal(
        new BN(0)
      );
      ...
      expect(await proposalContract.executedOption()).to.be.bignumber.equal(
        new BN(2)
      );

      // try to cancel vote
      await expectRevert(
        this.gov.cancelVote(defaultAcc, proposalID),
    ...
    it('checking proposal execution via call', async () => {
      ...
      expect(await proposalContract.executedMsgSender()).to.equal(
        this.gov.address
      );
      expect(await proposalContract.executedAs()).to.equal(
    ...
    it('checking proposal cancellation', async () => {
      ...
      await expectRevert(
        this.gov.cancelProposal(proposalID.add(new BN(1))),
        'proposal with a given ID doesnt exist'
      );
      await expectRevert(
        this.gov.cancelProposal(proposalID),
      ...
      const proposalStateInfoAfterTask = await this.gov.proposalState(
        proposalID
      );
      expect(proposalStateInfoAfterTask.winnerOptionID).to.be.bignumber.equal(
        new BN(0)
      );
      expect(proposalStateInfoAfterTask.votes).to.be.bignumber.equal(
        ether('0.0')
      );
      expect(proposalStateInfoAfterTask.status).to.be.bignumber.equal(
        new BN(4)
      );
    ...
    it('checking delegation vote creation', async () => {
      ...
      // make new vote
      evm.advanceTime(60);
      await this.govable.stake(firstVoterAcc, ether('10.0'), {
        from: firstVoterAcc
      });
      await this.gov.vote(firstVoterAcc, proposalID, choices0, {
        from: firstVoterAcc
      });
      await expectRevert(
        this.gov.vote(firstVoterAcc, proposalID, choices1, {
          from: delegatorAcc
        }),
        'zero weight'
      );
      ...
      await expectRevert(
        this.gov.vote(delegatorAcc, proposalID, choices1, {
          from: delegatorAcc
        }),
        'zero weight'
      );
      await expectRevert(
        this.gov.vote(otherAcc, proposalID, choices1, { from: delegatorAcc }),
        'zero weight'
      );
      await expectRevert(
        this.gov.vote(delegatorAcc, proposalID, choices1, {
      ...
      await expectRevert(
        this.gov.vote(firstVoterAcc, proposalID.add(new BN(1)), choices1),
        'proposal with a given ID doesnt exist'
      );
      await expectRevert(
        this.gov.vote(firstVoterAcc, proposalID, [new BN(3), new BN(4)], {
          from: delegatorAcc
        }),
        'wrong number of choices'
      );
      ...
      await this.gov.vote(firstVoterAcc, proposalID, choices1, {
        from: delegatorAcc
      });
      await expectRevert(
        this.gov.vote(
    ...
    var votersAndDelegatorsTests = (delegatorFirst) => {
      return async () => {
        ...
        beforeEach('create vote', async () => {
          ...
          if (delegatorFirst) {
            await this.govable.stake(firstVoterAcc, ether('30.0'), {
              from: delegatorAcc
            });
            await this.gov.vote(
              firstVoterAcc,
          ...
          await this.gov.vote(
            firstVoterAcc,
            proposalID,
            [new BN(3), new BN(2), new BN(0)],
            { from: firstVoterAcc }
          );
          await this.govable.stake(secondVoterAcc, ether('20.0'), {
            from: secondVoterAcc
          });
          await this.gov.vote(
            secondVoterAcc,
            proposalID,
            [new BN(2), new BN(3), new BN(4)],
            { from: secondVoterAcc }
          );
          if (!delegatorFirst) {
            await this.govable.stake(firstVoterAcc, ether('30.0'), {
        ...
        const checkFullVotes = async () => {
          const voteInfo1 = await this.gov.getVote(
            firstVoterAcc,
            firstVoterAcc,
            proposalID
          );
          ...
          const voteInfo2 = await this.gov.getVote(
            secondVoterAcc,
            secondVoterAcc,
            proposalID
          );
          ...
          const voteInfo3 = await this.gov.getVote(
            delegatorAcc,
            firstVoterAcc,
            proposalID
          );
          ...
          const voteInfo4 = await this.gov.getVote(
            firstVoterAcc,
            delegatorAcc,
            proposalID
          );
          ...
          expect(
            await this.gov.overriddenWeight(
              firstVoterAcc,
              proposalID.sub(new BN(1))
            )
          ).to.be.bignumber.equal(ether('0.0'));
          expect(
            await this.gov.overriddenWeight(firstVoterAcc, proposalID)
          ).to.be.bignumber.equal(ether('30.0'));
          const proposalStateInfo = await this.gov.proposalState(proposalID);
          expect(proposalStateInfo.winnerOptionID).to.be.bignumber.equal(
            new BN(0)
          );
          ...
          expect(option0.agreementRatio).to.be.bignumber.equal(
            ratio('0.533333333333333333')
          );
          expect(option1.agreementRatio).to.be.bignumber.equal(
            ratio('0.666666666666666666')
          );
          expect(option2.agreementRatio).to.be.bignumber.equal(
            ratio('0.733333333333333333')
          );
          const votingInfo = await this.gov.calculateVotingTally(proposalID);
          expect(votingInfo.proposalResolved).to.equal(true);
          expect(votingInfo.winnerID).to.be.bignumber.equal(new BN(2)); // option with a best opinion
        ...
        it('cancel votes', async () => {
          await this.gov.cancelVote(firstVoterAcc, proposalID, {
            from: firstVoterAcc
          });
          await this.gov.cancelVote(secondVoterAcc, proposalID, {
            from: secondVoterAcc
          });
          expect(
            await this.gov.overriddenWeight(firstVoterAcc, proposalID)
          ).to.be.bignumber.equal(ether('30.0'));
          const votingInfoAfter = await this.gov.calculateVotingTally(
            proposalID
          );
          ...
          await expectRevert(
            this.gov.cancelVote(firstVoterAcc, proposalID.add(new BN(1)), {
              from: delegatorAcc
            }),
            "doesn't exist"
          );
        ...
        it('checking voting state after delegator re-voting', async () => {
          ...
          await expectRevert(
            this.gov.cancelVote(firstVoterAcc, proposalID, {
              from: delegatorAcc
            }),
            "doesn't exist"
          );
        ...
        it('checking voting state after second voter re-voting', async () => {
          ...
          await expectRevert(
            this.gov.cancelVote(secondVoterAcc, proposalID, {
              from: secondVoterAcc
            }),
            "doesn't exist"
          );
        ...
        it('checking voting state after delegator recounting', async () => {
          this.govable.unstake(firstVoterAcc, ether('5.0'), {
            from: delegatorAcc
          });
          await this.gov.recountVote(delegatorAcc, firstVoterAcc, proposalID, {
            from: otherAcc
          });
          // check
          expect(
          ...
          expect(
            await this.gov.overriddenWeight(firstVoterAcc, proposalID)
          ).to.be.bignumber.equal(ether('25.0'));
          const proposalStateInfo = await this.gov.proposalState(proposalID);
          expect(proposalStateInfo.winnerOptionID).to.be.bignumber.equal(
            new BN(0)
          );
          ...
          expect(option0.agreementRatio).to.be.bignumber.equal(
            ratio('0.545454545454545454')
          );
          expect(option1.agreementRatio).to.be.bignumber.equal(
            ratio('0.672727272727272727')
          );
          expect(option2.agreementRatio).to.be.bignumber.equal(
            ratio('0.727272727272727272')
          );
          const votingInfo = await this.gov.calculateVotingTally(proposalID);
          expect(votingInfo.proposalResolved).to.equal(true);
          expect(votingInfo.winnerID).to.be.bignumber.equal(new BN(2)); // option with a best opinion
        ...
        it('checking voting state after first staker recounting', async () => {
          ...
          await this.gov.recountVote(firstVoterAcc, firstVoterAcc, proposalID, {
            from: otherAcc
          });
          // check
          expect(
          ...
          expect(option2.agreement).to.be.bignumber.equal(ether('44'));
          expect(option0.agreementRatio).to.be.bignumber.equal(
            ratio('0.571428571428571428')
          );
          expect(option1.agreementRatio).to.be.bignumber.equal(
            ratio('0.657142857142857142')
          );
          expect(option2.agreementRatio).to.be.bignumber.equal(
            ratio('0.628571428571428571')
          );
          const votingInfo = await this.gov.calculateVotingTally(proposalID);
          expect(votingInfo.proposalResolved).to.equal(true);
        ...
        it('checking voting state after cross-delegations between voters', async () => {
          ...
          await this.govable.stake(secondVoterAcc, ether('5.0'), {
            from: firstVoterAcc
          });
          await this.gov.vote(
            firstVoterAcc,
          ...
          await expectRevert(
            this.gov.recountVote(firstVoterAcc, firstVoterAcc, proposalID, {
              from: otherAcc
            }),
            'nothing changed'
          );
          await this.gov.recountVote(
            secondVoterAcc,
            secondVoterAcc,
            proposalID,
            { from: otherAcc }
          ...
          expect(
            await this.gov.overriddenWeight(firstVoterAcc, proposalID)
          ).to.be.bignumber.equal(ether('40.0'));
          expect(
            await this.gov.overriddenWeight(secondVoterAcc, proposalID)
          ).to.be.bignumber.equal(ether('0.0'));
          const proposalStateInfo = await this.gov.proposalState(proposalID);
          expect(proposalStateInfo.winnerOptionID).to.be.bignumber.equal(
            new BN(0)
        ...
        it('cancel votes via recounting', async () => {
          this.govable.unstake(firstVoterAcc, ether('10.0'), {
            from: firstVoterAcc
          });
          this.govable.unstake(secondVoterAcc, ether('20.0'), {
            from: secondVoterAcc
          });
          this.govable.unstake(firstVoterAcc, ether('30.0'), {
            from: delegatorAcc
          });
          await this.gov.recountVote(firstVoterAcc, firstVoterAcc, proposalID, {
            from: otherAcc
        ...
        afterEach('checking state is empty', async () => {
          ...
          await expectRevert(
            this.gov.recountVote(delegatorAcc, firstVoterAcc, proposalID, {
              from: otherAcc
            }),
            "doesn't exist"
          );
          await expectRevert(
            this.gov.recountVote(secondVoterAcc, secondVoterAcc, proposalID, {
              from: otherAcc
            }),
            "doesn't exist"
          );
    ...
    describe(
      ...
      votersAndDelegatorsTests(true)
    );
    describe(
      'checking votes for 2 self-voters and 1 delegation',
      votersAndDelegatorsTests(false)
    );
    ...
    it('checking voting with custom parameters', async () => {
      ...
      await this.govable.stake(defaultAcc, ether('10.0'));
      await expectRevert(
        this.gov.vote(defaultAcc, proposalID, [new BN(1)]),
        'wrong opinion ID'
    ...
    it('checking OwnableVerifier', async () => {
      const ownableVerifier = await OwnableVerifier.new(this.gov.address, {
        from: otherAcc
      });
      this.verifier.addTemplate(
        1,
        'plaintext',
        ownableVerifier.address,
        NonExecutableType,
        ratio('0.4'),
        ratio('0.6'),
        [0, 1, 2, 3, 4],
      ...
      const option = web3.utils.fromAscii('option');
      const proposal = await PlainTextProposal.new(
        'plaintext',
        'plaintext-descr',
        [option],
        ratio('0.5'),
      ...
      await ownableVerifier.createProposal(proposal.address, {
        value: this.proposalFee,
        from: otherAcc
      });
      await expectRevert(
